...
import { ColumnInventoryManagement, listInventoryManagement } from "./data/TableData";
...
export function InventoryManagementTable() {
    ...
    const headerColumns = React.useMemo(() => {
        if (visibleColumns === "all") return ColumnInventoryManagement;

        return ColumnInventoryManagement.filter((column) =>
            Array.from(visibleColumns).includes(column.uid)
        );
    }, [visibleColumns]);

    const filteredItems = React.useMemo(() => {
        let filteredUsers = [...listInventoryManagement];

        if (filterValue) {
            filteredUsers = filteredUsers.filter((user) =>
                user.im_patient_id.toLowerCase().includes(filterValue.toLowerCase())
            );
        }
            ...
            <div className="flex flex-col gap-4">
                ...
                <div className="flex justify-between items-center">
                    <span className="text-default-400 text-small">
                        Total {listInventoryManagement.length} inventory
                    </span>
                    <div className="flex gap-3">
                        <Dropdown>
                            ...
                            >
                                {ColumnInventoryManagement.map((column) => (
