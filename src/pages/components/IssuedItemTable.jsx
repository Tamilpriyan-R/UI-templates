...
import { ColumnIssuedItem, listIssuedItem } from "./data/TableData";
...
export function IssuedItemTable() {
    ...
    const headerColumns = React.useMemo(() => {
        if (visibleColumns === "all") return ColumnIssuedItem;

        return ColumnIssuedItem.filter((column) =>
            Array.from(visibleColumns).includes(column.uid)
        );
    }, [visibleColumns]);

    const filteredItems = React.useMemo(() => {
        let filteredUsers = [...listIssuedItem];

        if (filterValue) {
            filteredUsers = filteredUsers.filter((user) =>
                user.issue_patient_id.toLowerCase().includes(filterValue.toLowerCase())
            );
        }
            ...
            <div className="flex flex-col gap-4">
                ...
                <div className="flex justify-between items-center">
                    <span className="text-default-400 text-small">
                        Total {listIssuedItem.length} issued items
                    </span>
...
export default IssuedItemTable;
