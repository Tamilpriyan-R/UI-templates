...
import { ColumnLabReports, listLabReports } from "./data/TableData";
...
export function LabReportTable() {
    ...
    const headerColumns = React.useMemo(() => {
        ...
        return ColumnLabReports.filter((column) =>
            Array.from(visibleColumns).includes(column.uid)
        );
    }, [visibleColumns]);

    const filteredItems = React.useMemo(() => {
        let filteredUsers = [...listLabReports];

        if (filterValue) {
            filteredUsers = filteredUsers.filter((user) =>
                user.report_patient_id.toLowerCase().includes(filterValue.toLowerCase()) ||
                user.report_patient_id.toLowerCase().includes(filterValue.toLowerCase())
            );
        }
            ...
            <div className="flex flex-col gap-4">
                ...
                <div className="flex justify-between items-center">
                    <span className="text-default-400 text-small">
                        Total {listLabReports.length} lab reports
                    </span>
