...
import { ColumnMedicinePurchaseEntry, listMedicinePurchaseEntry } from "./data/TableData";
...
export function MedicinePurchaseEntryTable() {
    ...
    const headerColumns = React.useMemo(() => {
        if (visibleColumns === "all") return ColumnMedicinePurchaseEntry;

        return ColumnMedicinePurchaseEntry.filter((column) =>
            Array.from(visibleColumns).includes(column.uid)
        );
    }, [visibleColumns]);

    const filteredItems = React.useMemo(() => {
        let filteredUsers = [...listMedicinePurchaseEntry];

        if (filterValue) {
            filteredUsers = filteredUsers.filter((user) =>
                user.med_item_name.toLowerCase().includes(filterValue.toLowerCase())
            );
        }
            ...
            <div className="flex flex-col gap-4">
                ...
                <div className="flex justify-between items-center">
                    <span className="text-default-400 text-small">
                        Total {listMedicinePurchaseEntry.length} medicine
                    </span>
                    <div className="flex gap-3">
                        <Dropdown>
                            ...
                            >
                                {ColumnMedicinePurchaseEntry.map((column) => (
...
export default MedicinePurchaseEntryTable;
