...
import { ColumnRadiologyReports, listRadioReports } from "./data/TableData";
...
export function RadiologyReportTable() {
    ...
    const headerColumns = React.useMemo(() => {
        if (visibleColumns === "all") return ColumnOpPatient;

        return ColumnRadiologyReports.filter((column) =>
            Array.from(visibleColumns).includes(column.uid)
        );
    }, [visibleColumns]);

    const filteredItems = React.useMemo(() => {
        let filteredUsers = [...listRadioReports];

        if (filterValue) {
            filteredUsers = filteredUsers.filter((user) =>
                user.radio_rep_id.toLowerCase().includes(filterValue.toLowerCase()) ||
                user.radio_patient_id.toLowerCase().includes(filterValue.toLowerCase())
            );
        }
            ...
            <div className="flex flex-col gap-4">
                ...
                <div className="flex justify-between items-center">
                    <span className="text-default-400 text-small">
                        Total {listRadioReports.length} radiology reports
                    </span>
                    <div className="flex gap-3">
                        <Dropdown>
                            ...
                            >
                                {ColumnRadiologyReports.map((column) => (
