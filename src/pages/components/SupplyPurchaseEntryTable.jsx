...
import { ColumnSupplyPurchaseEntry, listSupplyPurchaseEntry } from "./data/TableData";
...
export function SupplyPurchaseEntryTable() {
    ...
    const headerColumns = React.useMemo(() => {
        if (visibleColumns === "all") return ColumnSupplyPurchaseEntry;

        return ColumnSupplyPurchaseEntry.filter((column) =>
            Array.from(visibleColumns).includes(column.uid)
        );
    }, [visibleColumns]);

    const filteredItems = React.useMemo(() => {
        let filteredUsers = [...listSupplyPurchaseEntry];

        if (filterValue) {
            filteredUsers = filteredUsers.filter((user) =>
                user.sup_item_name.toLowerCase().includes(filterValue.toLowerCase()) 
            );
        }
            ...
            <div className="flex flex-col gap-4">
                ...
                <div className="flex justify-between items-center">
                    <span className="text-default-400 text-small">
                        Total {listSupplyPurchaseEntry.length} supply
                    </span>
                    <div className="flex gap-3">
                        <Dropdown>
                            ...
                            >
                                {ColumnSupplyPurchaseEntry.map((column) => (
...
export default SupplyPurchaseEntryTable;
